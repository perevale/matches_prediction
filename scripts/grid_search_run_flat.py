...
acc = run_flat_cont(input_filename, dir_prefix=sys.argv[3], exp_num=int(exp_counter),
                     embed_dim=int(parameters["embed_dim"]),
                     n_dense=int(parameters["n_dense"]), dense_dims=to_int(parameters["dense_dims"]))

results = "model=|{}| EXP:|{}| embed_dim=|{}| n_dense=|{}| dense_dims=|{}| achieved accuracy:|{}|\n" \
    .format("Flat", exp_counter, parameters["embed_dim"], parameters["n_dense"],
